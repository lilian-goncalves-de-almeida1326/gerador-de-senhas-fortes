...
/**
    ...
    // Recupera automaticamente, se existir
    const usuarioSalvo = localStorage.getItem('usuario');
    if (usuarioSalvo) {
      input.value = usuarioSalvo;
    }
 */
...
/**
function exportarSenhaComoTxt(senha) {
  // 1. Cria o conteúdo como texto puro
  const blob = new Blob([senha], { type: "text/plain" });

  // 2. Cria um link temporário no navegador apontando para o arquivo
  const link = document.createElement("a");
  link.href = URL.createObjectURL(blob);
  link.download = "senha-segura.txt"; // nome do arquivo

  // 3. Clica no link automaticamente para iniciar o download
  link.click();

  // 4. Limpa o link da memória depois
  URL.revokeObjectURL(link.href);
}
 */